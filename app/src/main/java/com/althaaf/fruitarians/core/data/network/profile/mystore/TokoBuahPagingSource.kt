...
class TokoBuahPagingSource(private val apiService: ApiService, private val dataStore: UserPreference): PagingSource<Int, BuahItem>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, BuahItem> {
        return try {
            ...
            val responseData = apiService.getTokoBuah(token = "${dataSessionUser.tokenType} ${dataSessionUser.accessToken}", page = page, size = 2)
            val responseListData: List<BuahItem> = responseData.buah ?: emptyList()
